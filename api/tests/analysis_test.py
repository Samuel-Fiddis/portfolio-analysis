...
from analysis import get_averages, get_geometric_mean, get_standard_deviation, get_covariance_matrix, get_correlation_matrix, get_portfolio_drawdown_percentage, get_symbols_drawdown_percentage
...
class TestAnalysis:
    ...
    @pytest.mark.parametrize("symbol,expected_drawdown", [
        ('MSFT', -30.52),
        ('AAPL', -30.46),
        ('DELL', -41.33),
    ])
    def test_symbols_drawdown_percentage(self, optimisation_test_data, symbol, expected_drawdown):
        drawdowns = get_symbols_drawdown_percentage(optimisation_test_data)
        assert drawdowns[symbol]['max_drawdown']['percent'] == pytest.approx(expected_drawdown, rel=1e-3)
    ...
    def test_portfolio_drawdown_percentage(self, optimisation_test_data):
        ...
        portfolio_drawdown = get_portfolio_drawdown_percentage(optimisation_test_data, weights)
        assert portfolio_drawdown['max_drawdown']['percent'] == pytest.approx(-30.10, rel=1e-3)
